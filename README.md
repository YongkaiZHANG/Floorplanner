...
Ruler mode supports grid and object-edge snapping. Press `o` for orthogonal measurement and click two snapped points to create a ruler. Matching edge directions override the cursor’s dominant travel: two vertical IP edges report their left/right X separation, and two horizontal edges report their top/bottom Y separation, even when the IP projections do not overlap. Double-clicking a block in Select mode opens and pins its Properties panel; double-clicking empty canvas fits the view.
