...
Loading the file calls `FPCreateFloorplan()` automatically. It creates or overwrites the generated `layout` cellviews using `maskLayout`, creates real `prBoundary` objects, and opens masters read-only. Every generated IP master contains centered name and `width x height um` labels on `("text" "drawing")`; their text heights adapt to the IP height, available width, and label length. Each non-pad IP gets a placed wrapper named `<TopCell>_<Instance>_PLACED`; that wrapper contains the original master at the exact canvas coordinate and orientation. The Top Cell instantiates every wrapper at `(0,0)` with `R0`. All pads are contained in one `<TopCell>_PAD_BANK` cell, also instantiated once at `(0,0)`.
